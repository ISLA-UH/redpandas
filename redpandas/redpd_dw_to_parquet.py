...
def redpd_df_from_config(config: RedpdConfig,
                         export_dw_pickle: bool = True,
                         export_df_parquet: bool = True,
                         create_dw: bool = True,
                         print_dq: bool = False,
    ...
    """
    redpd_df(input_dir=config.input_dir,
...
def redpd_df(input_dir: str,
             ...
             highpass_type: str = 'obspy',
             frequency_filter_low: float = 1./rpd_scales.Slice.T100S,
             filter_order: int = 4) -> Tuple[Union[str, None], pd.DataFrame]:
    """
    Extract RedVox data, convert to pandas dataframe

    :param input_dir: string, directory that contains the files to read data from.  REQUIRED
    :param create_dw: create RedVox DataWindow to load RedVox raw data. Default if True. If false, open existing pickle
        file
    :param export_dw_pickle: export created RedVox DataWindow to pickle if True. Default is True
    :param export_df_parquet: export created RedPandas Dataframe to parquet if True. Default is True
    ...
    :param frequency_filter_low: apply highpass filter. Default is 100 second periods
    :param filter_order: the order of the filter integer. Default is 4
    :return: string with output directory if exporting pickle, else None, and pd.DataFrame
    """
    ...
    if create_dw:
        ...
        rdvx_data = rpd_dw.build(api_input_directory=input_dir,
                                 export_pickle=export_dw_pickle,
    ...
    if export_df_parquet is True:  # export dataframe to parquet
        ...
        full_output_dir_path_parquet = os.path.join(output_dw_pqt_dir, output_filename_pqt)
        df_all_sensors_all_stations.to_parquet(full_output_dir_path_parquet)
        ...
        return full_output_dir_path_parquet, df_all_sensors_all_stations
        ...
        return None, df_all_sensors_all_stations
