...
# # TODO: Clean up
# def plot_wiggles_pandas_align(df: pd.DataFrame,
#                               align_epoch_s: float,
#                               sig_wf_label: str,
#                               sig_sample_rate_label: str,
#                               sig_id_label: str,
...
#
#     if sig_timestamps_label is not None:
#         # Add cross-correlation time shift
#         epoch_j = np.zeros(wiggle_num)
#         for i, j in enumerate(df.index):
#             epoch_j[i] = df[sig_timestamps_label][j].min() + df[offset_seconds_label][j]
#         time_epoch_origin = np.min(epoch_j)
#     else:
#         time_epoch_origin = 0.
#
...
#         # Add cross-correlation time shift
#         if sig_timestamps_label is None:
#             time_s = np.arange(len(df[sig_wf_label][j])) / df[sig_sample_rate_label][j] + df[offset_seconds_label][j]
#         else:
#             time_s = df[sig_timestamps_label][j] - time_epoch_origin + df[offset_seconds_label][j]
#
#         sig_j = df[sig_wf_label][j] / np.max(df[sig_wf_label][j])
#         ax1.plot(time_s, sig_j + wiggle_offset[i], color=wf_color)
#         xlim_min[j] = np.min(time_s)
